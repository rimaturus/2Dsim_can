...
volatile float offset_x = 0.5f;
volatile float offset_y = 0.5f;


void load_cones_positions(const char *filename, cone *cones, int max_cones)
{
    // Ensure we use the "C" locale so that '.' is the decimal separator
        ...
        switch (event.type) {
            case YAML_SCALAR_EVENT:
                if (state == 0) {
                    ...
                    if (strlen(current_key) == 0) {
                        // Store the key (e.g., "x", "y", or "color")
                        ...
                        // Process the value corresponding to the current key
                        if (strcmp(current_key, "x") == 0) {
                            char *endptr = NULL;
                            errno = 0;

                            float val = strtof((char *)event.data.scalar.value, &endptr);

                            if (endptr == (char *)event.data.scalar.value || errno == ERANGE) {
            ...
            case YAML_MAPPING_END_EVENT:
                if (state == 3) {
                    ...
                    state = 2; // Return to the 'cones' sequence state
...
#ifdef DEBUG
    for (int j = 0; j < i; j++) {
        printf("Cone %d: x=%.2f, y=%.2f, color=%d\n", j, cones[j].x, cones[j].y, cones[j].color);
    }
#endif /* DEBUG */
}

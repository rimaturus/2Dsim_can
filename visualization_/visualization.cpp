...
std::vector<Cone>	loadCones(const std::string &filename)
{
	...
	try
	{
		conesNode	= YAML::LoadFile(filename)["cones"];
		if (!conesNode)
		{
...
void	updateCarPositionSingleTrackModel(Car &car, float deltaTime, std::mutex &carMutex)
{
	...
	beta = atan2(tan(delta), 2.0f);  // Assuming front-wheel steering
...
int	main()
{
	...
	canReceiveThread = std::thread(receiveCANMessagesThread, can_socket, std::ref(done), std::ref(car), std::ref(carMutex));

	// Start the send car data thread
	canSendThread = std::thread(sendCarDataThread, send_can_socket, std::ref(car), std::ref(carMutex), std::ref(done));
